...
class TelloNode():
    def __init__(self, node: Node):
        ...
        self.network_interface = (self.node.get_parameter('network_interface').value)
        self.video_foward_port = int(self.node.get_parameter('video_foward_port').value)
        self.stream_on = bool(self.node.get_parameter('stream_on').value)
        ...
        if len(self.camera_info_file) == 0:
            lib_directory = ament_index_python.get_package_prefix(
                'tello_py_driver') + '/lib/tello_py_driver'
            self.camera_info_file = lib_directory + '/ost.yaml'
        ...
        self.tello = Tello("192.168.10.1", self.network_interface, self.video_foward_port)
        self.tello.connect()
            ...
            while True:
                ...
                if self.tf_pub:
                    ...
                    t.transform.translation.z = (
                        self.tello.get_barometer()) / 100.0
                ...
                if self.pub_odom.get_subscription_count() > 0:
                    ...
                    odom_msg.twist.twist.linear.x = float(
                        self.tello.get_speed_x()) / 100.0
                    odom_msg.twist.twist.linear.y = float(
                        self.tello.get_speed_y()) / 100.0
                    odom_msg.twist.twist.linear.z = float(
                        self.tello.get_speed_z()) / 100.0
        ...
        def status_loop():
            while True:
                ...
                if self.pub_status.get_subscription_count() > 0:
                    ...
                    msg.barometer = int(self.tello.get_barometer() * 100.0)
    ...
    def start_video_capture(self, rate=1.0/30.0):
        ...
        def video_capture_thread():
            while True:
                cv2_frame = self.tello.get_frame_read()

                if cv2_frame is None:
                    self.node.get_logger().info("No frame received, trying again in 5 seconds")
                    time.sleep(5)
                    continue

                time.sleep(0.005) # without this sleep video freezes some how

                msg = self.bridge.cv2_to_imgmsg(numpy.array(cv2_frame), 'bgr8')
                msg.header.frame_id = self.tf_drone
...
def main(args=None):
    ...
    node = rclpy.create_node('tello_py_driver')
    drone = TelloNode(node)

    rclpy.spin(node)

    drone.terminate('Shutting down')
    rclpy.shutdown()
